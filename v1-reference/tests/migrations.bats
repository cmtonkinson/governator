...
@test "migration moves last_update_at into config and removes file" {
  printf '%s\n' "12345" > "${REPO_DIR}/_governator/_local_state/last_update_at"
  ...
  [ "$status" -eq 0 ]
  [ "${output}" = "12345" ]
  [ ! -f "${REPO_DIR}/_governator/_local_state/last_update_at" ]
}
