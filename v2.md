...
1. `gov init` -> creates `_governator/` and its layout (`_durable_state`, `_local_state`, roles, custom prompts, docs,
   plan prompts, templates, etc.).
...
The phase tracker is stored under `_governator/_durable_state/phase-state.json`. Each `gov run` reads that file,
