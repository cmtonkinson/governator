...
## Objective
Assess the current state of project code (if any) against documented intent/designs/requirements/constraints/assumptions
and generate a normalized Decision Ledger, Gap Register, and Planning Constraints.

Your task is to empower project planning by analyzing what decisions are already locked, what information is
missing/ambiguous/conflicting, and what rules must be observed while synthesizing the project plan. Your deliverables
provide the necessary scaffolding for planning such that risks and unknowns are documented (and can be mitigated) as
early as possible.
