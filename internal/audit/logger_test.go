...
func TestLoggerWritesEntries(t *testing.T) {
	...
	if err := logger.LogWorktreeCreate("T-014", "worker", ".governator/worktrees/task-T-014", "task-T-014"); err != nil {
	...
	expectedFirst := "ts=2025-01-14T19:02:11Z task_id=T-014 role=worker event=worktree.create path=.governator/worktrees/task-T-014 branch=task-T-014"
...
func TestLogWorkerTimeout(t *testing.T) {
	...
	if err := logger.LogWorkerTimeout("T-042", "worker", 300, ".governator/worktrees/task-T-042"); err != nil {
	...
	expected := "ts=2025-01-14T20:15:30Z task_id=T-042 role=worker event=worker.timeout timeout_seconds=300 worktree_path=.governator/worktrees/task-T-042"
	if lines[0] != expected {
		t.Fatalf("expected audit line %q, got %q", expected, lines[0])
	}
}
