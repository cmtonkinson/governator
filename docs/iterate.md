...
```
cd ~/repo/governator-TEST/ && rm -rf _governator .git && git init . && git add . && git commit -m 'Initial commit' && gov -v init && gov -v start && gov -v status
```
...
After you execute `gov start`, get the pid of the worker and watch it - this saves polling loops and tokens.
